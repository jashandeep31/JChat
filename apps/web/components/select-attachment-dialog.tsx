...
export function SelectAttachmentDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] rounded-xl shadow-2xl">
        <DialogHeader>
          ...
          {getAttachmentsQuery.isLoading ? (
            ...
            getAttachmentsQuery.data?.map((attachment) => (
              <div
                key={attachment.id}
                className={cn(
                  "mb-3 p-3 border rounded-lg cursor-pointer hover:bg-muted/50 transition-colors duration-200 relative",
              ...
              >
                <div className="flex items-start gap-3">
                  <div className="flex-shrink-0 p-2 bg-muted/30 rounded text-muted-foreground">
